...
function DashboardContent() {
  ...
  function handleSubmit() {
    ...
    axios({
      method: "post",
      url: `${url}/api/v1/app/heros`,
      data: data,
    })
      .then((res) => {
        axios
          .put(`${url}/api/v1/app/heros/image/${res.data.data.id}`, formData)
          .then((res) => {
    ...
    /*    axios
      .put(`${url}/api/v1/app/heros/`, formData, {
        headers: {
      ...
      .then((res) => {
        if (res.data.status === 404) {
          ...
          setSuccessMessage("Hero Product Creation successful!");
        }
      }); */
  }
  ...
  /*  function handleUpdate(heroId) {
    ...
    axios.put(`${url}/api/v1/app/heros/${heroId}`, formData).then((res) => {
      if (res.data.status === 404) {
        setErrorMessage("Hero product creation failed");
      } else if (res.data.status === 200) {
        setSuccessMessage("Hero Product Creation successful!");
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={4}>
                <Paper sx={{ p: 2, display: "flex", flexDirection: "column" }}>
                  ...
                  {/*    <TextField
                    id="multiline"
                    value={HeroProductDescription}
                    onChange={(e) => setHeroProductDescription(e.target.value)}
                    label="Description"
                    minRows={5}
                    multiline
