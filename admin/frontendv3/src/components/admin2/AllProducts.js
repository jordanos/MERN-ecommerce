...
function AllProducts() {
  ...
  useEffect(() => {
    axios.get(`${url}/api/v1/products`).then((res) => {
  ...
  function handeldeDelete(ProductId) {
    ...
    axios
      .delete(`${url}/api/v1/products/${ProductId}`, { data })
      .then((res) => {
  ...
  function handleSearch(e) {
    ...
    var result = allProductsData.filter((result) => {
      return (
        result.name.match(new RegExp(`${e.target.value}`, "i")) ||
        result.name.match(new RegExp(`${e.target.value}`, "i"))
      );
    });
            ...
            <CardActionArea component="a" href="#">
             ...
             Category:  {row.categories}
                ...
                <CardContent sx={{ flex: 1 }}>
                  ...
                  <Typography variant="subtitle1" color="text.secondary">
                    Posted Date: <Timestamp relative date={row.createdAt} />
                  </Typography>
                  <Typography variant="subtitle1" color="text.secondary">
                  ...
                  <Typography>
                    ...
                    {row && row.istrendingproduct === 0 ? (
                      <>
                        <Button
                          onClick={(e) => handelesetasTrending(row.productid)}
                          variant="outlined"
                          color="success"
                        >
                          Set As Trending
                      ...
                      <>
                        <Button
                          onClick={(e) =>
                            handeleremoveasTrending(row.productid)
                          }
                          variant="outlined"
                          color="error"
                        >
